...
public class HtmlFetcher {
	...
	public static boolean isHtml(Map<String, List<String>> headers) {
		if (headers.containsKey("Content-Type")) {
			return headers.get("Content-Type").get(0).contains("text/html");
		}
		return false;
	}
	...
	public static int getStatusCode(Map<String, List<String>> headers) {
		if (headers.containsKey(null)) {
			String[] status = headers.get(null).get(0).split(" ");
	...
	public static boolean isRedirect(Map<String, List<String>> headers) {
		return getStatusCode(headers) >= 300 && getStatusCode(headers) <= 399;
	}
